...
- **批次 4 篇分頁隔離法**：cmd+v 用單一系統剪貼簿＋全域焦點，4 分頁不能同時貼。做法＝開 4 獨立分頁各自 new-story，貼上步驟在分頁間「快速輪序、不重疊」（一篇 title+body 貼完才換下一篇載剪貼簿）。隔離的好處是某分頁 editor 降級不影響其他三個。鎖定 recipe（2026-06-13 一批 4 篇零失敗）：① `printf title|pbcopy` ② JS 取 `.graf--title` rect ③ click→cmd+a→Delete→cmd+v（標題一次到位、draft 建立）④ 驗 title ⑤ `clipboard-html.sh` body ⑥ **Return→cmd+v**（Return 建新塊避開吸併）⑦ 驗 p0/結尾/佔位符。
