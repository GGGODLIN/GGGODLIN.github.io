...
test("approved connector tags expose stable display metadata", () => {
  ...
  for (const expectedEntry of expectedConnectorEntries) {
    const entry = entriesById.get(expectedEntry.id);
    assert.ok(entry, expectedEntry.id);
    assert.equal(entry.label, expectedEntry.label);
    assert.deepEqual(entry.aliases, []);
    assert.equal(entry.dimension, "subject");
    assert.equal(typeof entry.meaning, "string");
    assert.ok(entry.meaning.length > 0, expectedEntry.id);
    assert.equal(typeof entry.boundary, "string");
    assert.ok(entry.boundary.length > 0, expectedEntry.id);
  }
});
