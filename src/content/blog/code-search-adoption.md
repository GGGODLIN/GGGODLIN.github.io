...
而「agent 對不熟的 MCP 工具保守、退回 grep」這件事，不是我一個專案的孤例。[ast-grep 的維護者](https://ast-grep.github.io/blog/ast-grep-agent.html)記錄過 Gemini 寧可自己編 CLI 指令、也不用它提供的 MCP 工具；[Hacker News](https://news.ycombinator.com/item?id=47715000) 上也吵過 MCP 跟 CLI 工具到底該怎麼選；上面那篇論文同樣談這件事。根因大致兩個：模型對 grep 這種老工具太熟，加上沒有明確指令叫它換工具（工具一多，光把每個工具的說明塞進對話，就開始拖累判斷）。
