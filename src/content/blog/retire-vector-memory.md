...
- **實際使用主動搜尋：3 次**（cmux 問題、TanStack Start 用法、session 備份方案）
- **drawer 累積：3429 個**
- **搜尋 / drawer 比：0.09%**（3 次搜尋 ÷ 3429 個 drawer）
...
退役之前另外挑了 6 個退役驗證查詢（都是真實工作會問的問題），同時送 mempalace MCP 跟直接 grep MEMORY.md，看哪個撈得到：
...
回收：271 MB 磁碟（245 MB uv venv + 26 MB 向量資料），幾分鐘的事。（前面膨脹到 84GB 是早期 mining 範圍沒設好的誤掃、當時就用 symlink script 處理掉了；退役當天實際佔用就是這 271 MB。）
