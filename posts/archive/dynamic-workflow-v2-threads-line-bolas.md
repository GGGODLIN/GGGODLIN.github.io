---
title: "deep-research workflow 撞了 server burst 限流"
...
最近把 [dynamic workflows](https://claude.com/blog/introducing-dynamic-workflows-in-claude-code)（2026-05-28 GA）拿來密集跑研究任務，功能怎麼運作先看先發 B 篇「[ultracode workflow，別跑完就丟](https://gggodlin.github.io/blog/workflow-vs-skill/)」，這篇聚焦一個我親身踩到的坑：官方 `deep-research` workflow 在 Opus 4.8 端點跑，撞了 server-side burst 限流，75 個 verify agent 擠在同一波送出去，整支 workflow 直接 fail。
...
我自己寫過一支 `deep-research` skill（`~/.claude/skills/deep-research`），8 個階段的研究管線，有 citation 追蹤、可以出 PDF 交付報告。Claude Code 二進位裡也有一支官方的 `deep-research` named workflow，是內建進去的，檔案系統找不到，只能用名字呼叫。

這兩個撞名了，但兩條通道完全獨立、互不感知——Skill tool 走 skill、Workflow tool 走 named workflow。我以為在叫官方版，實際上叫到哪支要看 routing 怎麼設。解法是在 engine 加一道 routing gate，明確指定要走哪條通道。撞名問題修掉後，才正式開始密集用官方版，然後撞到真正的牆。

---

## 官方 deep-research 的 fan-out 有多大
...
**場景說明：以下數字全部適用「官方 deep-research workflow + Opus 4.8 端點」這個組合，不是作者自寫腳本的行為，換其他端點或其他 workflow 結果可能差異很大。**
...
官方腳本裡面 0 個 model override，所有 subagent 全部繼承 session 主模型。也就是說，在 Opus session 下跑，75 個 verify agent 全部打 Anthropic 的 Opus 端點。

同一支腳本、同一個題目，換到另一個端點（mimo），跑了 66 分鐘、0 撞限。Opus session 版 7 分鐘跑完（但是 fail 的）。

關鍵不是模型貴不貴、快不快，是 server 端的限流政策。Opus 走的是 Anthropic 的端點，server 端有 acceleration limit 這種 burst 限流，fan-out 一大就撞；mimo 是完全不同公司的端點，server 端政策不一樣，同一支腳本就不撞。撞不撞，看你 session 連到哪一家的 server。
...
時間變長了，但品質回來了。重點不是跑慢，是把峰值並發壓在限流線以下；批次大小視端點調整，2 不是通案。

---

## 為什麼從自寫 skill 轉向官方 workflow

我自己寫的 deep-research skill 功能不差，8 個階段、citation 追蹤、可以出 PDF、管線清楚。但驗證是單管線，找到支持就過，沒有對抗性。
...
腳本確定性是 dynamic workflows 的賣點，但 fan-out 規模由腳本決定之後，burst 密度問題也跟著進來了。官方設計的 75 agent 一次湧入，落到有 acceleration limit 的 Anthropic 端點，就從一個編排問題變成了 server burst 問題。
