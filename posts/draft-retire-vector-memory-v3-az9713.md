---
title: 跑一個半月的 vector DB memory 我拔了——curation 比 algorithm 重要
slug: retire-vector-memory-az9713
date: 2026-05-17
status: draft (v3 — full az9713 skill flow)
flow: az9713 (voice-dna-creator + thought-leadership skill)
inputs:
  - voice-dna.json
  - thought-leadership SKILL.md (full spec compliance)
  - quality checklist 8 條
  - output format (TITLE / TYPE / TAKEAWAYS / SNIPPETS)
---

TITLE: 跑一個半月的 vector DB memory 我拔了——curation 比 algorithm 重要
TYPE: Perspective Piece
TARGET LENGTH: 1300 字
TARGET PLATFORM: blog.gggodlin.com

---

# 跑一個半月的 vector DB memory 我拔了——curation 比 algorithm 重要

最近把跑了一個半月的 vector DB memory 系統 mempalace 拔了。

省流：6 個 sanity query 對比 mempalace MCP vs grep MEMORY.md，vector **0/6**、grep **6/6**。HNSW 不是壞、ChromaDB 也不是雷——是 **curation 比 retrieval algorithm 重要**這件事，在我自己的 corpus 上被狠狠驗證了一次。
...
我以為自己會用，**沒想到**根本沒打開過——hook 自己跑得很勤，retrieval 介面我從沒主動觸發。Claude 也沒主動 query，因為它有更直接的方式拿 context（grep MEMORY.md）。
...
不是「vector 命中率比較低」——**是 vector 沒一個答對**。同樣 query，grep 6/6 完勝。

## Curation 比 algorithm 重要

冷靜分析，問題不在 HNSW、也不在 embedding model，**而在 corpus 本身**：

- **mempalace drawers** = raw session 對話片段，沒 curation，命中也只是一段沒結論的 context snippet——典型的「**幻覺滿天飛**」
- **MEMORY.md entries** = 我手寫的「結論濃縮」，自帶答案

File-based hand-curated memory 同時贏兩層：

1. **Retrieval 層**：dev workflow 的 query 多含 entity name（套件名 / 工具名 / 專案名），keyword grep 對精確 token 比 vector 強——這是 entity 查找的天下，**vector 在這個場景上找不到就業機會**
2. **Content 層**：每條 entry 是手動 distill 過的 summary，不是 raw transcript

**反觀**換 hybrid retrieval（Mem0、Memori 那類）也救不了——hybrid 只解 retrieval 那一半問題，corpus 是 raw session 這件事改不掉。除非 mining pipeline 改成「LLM 抽取 → 寫精華 chunk」——但那就是現在直接寫 hand-curated memory 本身在做的事，vector DB 變成多餘一層。

**貴的東西，唯一的問題就只有貴而已。** mempalace 不貴，但它**撐不起**我這 corpus 的 query pattern。
...
**終究是**主流從 vector retrieval 改走 cron-based consolidation。

業界沒在優化「怎麼讓 vector 撈得更準」，而是優化「怎麼定期把 raw session 蒸餾成 curated chunk」——剛好是我退役 mempalace 後得到的同一個結論：**curation > retrieval algorithm**。我自己的 `social-info/scripts/local-analysis/` 走 launchd cron + `claude -p` daily 跑 audit，方向跟 Anthropic 內部設計、OpenClaw 主流一致。也算是老問題了。
...
| 因果 / 為什麼 | embedding 對 reason vs fact 沒分別——常常**跑偏** |
| 環境事實 / entity 查找 | grep |
| 概念連接 | vector 有部分價值 |
...
KEY TAKEAWAYS:
1. **vector DB 退役不是技術問題，是 corpus 問題**——raw session 當 corpus 餵給 vector store，retrieval 跟 content 兩層都輸給 hand-curated MEMORY.md
2. **curation > retrieval algorithm**——HNSW 是業界標準但解決不了「沒 distill 過的 chunk 撈出來也沒答案」這件事
3. **業界 2026-05 已轉 cron-based consolidation**（CC 內藏 dream / Anthropic Dreams API / OpenClaw / dream-skill）印證同方向；個人 setup 的 daily routine + hand-curated MEMORY.md 已走在主流前面

SUGGESTED SOCIAL SNIPPETS:
- 「6 個 sanity query，mempalace MCP 拿到 0/6，grep 拿 6/6。HNSW 不是壞，是 curation 比 algorithm 重要。」
- 「Read:Write = 0.09%。寫 928 次、讀 3 次。我自建的 vector memory 是個 dark system。」
- 「業界 2026-05 改走 cron consolidation，不是 vector retrieval。終究是 curation > algorithm。」
